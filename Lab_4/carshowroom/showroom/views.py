...
def product_detail(request, id, slug):
    ...
    return render(
        request,
        "showroom/product/detail.html",
        {"product": product, "cart_product_form": cart_product_form},
    )


def statistics(request):
    total_revenue = Order.objects.aggregate(total_revenue=Sum("items__price"))[
        "total_revenue"
    ]
...
class ProductUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if stock < 0:
            form.add_error(
                "stock", "Количество на складе должно быть неотрицательным числом."
            )
